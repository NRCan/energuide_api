...
def upload_file() -> str:
    ...
    if request.form['signature'] != signed_file:
        App.logger.error(f"Sig sent: {request.form['signature']} != {signed_file}")
        abort(404)
    ...
    if upload_stream_to_azure(azure_sc, file, filename):
...
if __name__ == "__main__":
    ...
    Handler = handlers.RotatingFileHandler('extract_endpoint.log', maxBytes=10000000, backupCount=5)
